...
**선금이 들어오는 두 경로** — 공공기관(구청·시청 등) 선금은 담당자(서무)가 보낸 명단이 **자동 등록**으로 들어오고, 회사(사기업)·개인 선금은 사장님이 **수동 등록**으로 직접 입력합니다. 두 경로 모두 한 장부에서 함께 관리되며, 직원마다 `소속`(선택)으로 구분합니다. **돈은 앱 밖에서 움직입니다** — 선금 결제(카드결제가 다수, 계좌이체도 있음)는 선금을 보내는 쪽과 음식점이 직접 진행하고, 운영자는 자금을 수수·보관·이체·정산하지 않습니다. 담당자가 CSV 하나로 여러 음식점에 명단을 보내더라도 **결제는 음식점별로 각각** 일어납니다.

## 주요 기능
- 소속·부서별 직원 잔액 관리 (`소속`은 선택 입력 — 회사명 또는 '개인')
- 공공기관 부서 목록 관리 (전국 시도→기관 2단 선택, 직접 입력도 가능)
- 자동 등록: 공공기관 담당자(서무)가 보낸 암호화 명단 수신·승인(사장님이 받은 금액을 카드 매출전표 또는 통장 입금 내역과 대조한 뒤 승인)
