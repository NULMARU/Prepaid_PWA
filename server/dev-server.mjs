...
// 메모리 저장 + 목 LOCALDATA. 실제 LOCALDATA 키가 있으면 LOCALDATA_KEY 환경변수로 실호출.
//   node server/dev-server.mjs            # 목 검색
//   LOCALDATA_KEY=<키> node server/dev-server.mjs   # 실제 검색
...
const env = {
  ALLOW_ORIGIN: '*',
  LOCALDATA_KEY: process.env.LOCALDATA_KEY || '',
  LOCALDATA_BASE: process.env.LOCALDATA_BASE,
  LOCALDATA_OPNSVCID: process.env.LOCALDATA_OPNSVCID
};
if (!env.LOCALDATA_KEY) {
