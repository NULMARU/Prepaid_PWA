...
이 리포를 **처음 받은 개발자**를 위한 문서 묶음이다. 최종 갱신 **2026-09-12 (beta.48)**, 백업 `schemaVersion 3`, IndexedDB `VER 3`.
현재 앱 버전은 `index.html`의 `APP_VERSION` 상수에서 직접 읽을 것(활발히 올라가는 중이다).
...
- **중복하지 않는다.** 이미 정확한 원본이 있는 것은 링크만 한다.
  ...
  | D1 스키마(컬럼 주석 포함) | `server/schema.sql`(신규 설치용 전체 스키마) |
  | D1 마이그레이션 이력 | `server/migrations-2026-07.sql` · `-2026-08.sql` · `-2026-09.sql` (**월별 파일, 각각 append-only**) |
...
```bash
npm i                          # playwright 설치
bash harness/verify-all.sh     # 12/12 기대 (약 2~4분)
```
- 실패 시 로그: `/tmp/v1.log` ~ `/tmp/v5.log`.
...
5. [04-contracts.md](04-contracts.md) — 깨면 조용히 망가지는 계약 14개. 특히 **C1(`batch_hash`)·C2(`district`)** 는 실제 라이브 장애 이력이 있고, **C9~C14**(2026-09)는 "같은 돈이 두 번 들어가는 길"을 막는 것들이다.
...
| [03-data-model.md](03-data-model.md) | IndexedDB 스토어·필드, 해시 체인, 잔액 규칙, 백업 포맷(평문·**잠긴 백업**)·버전 정책, `meta` 키, D1 요약, **오리진 종속성** | 데이터를 만질 때 |
| [04-contracts.md](04-contracts.md) | 깨면 안 되는 계약 14개 + **깨졌을 때 무슨 일이 벌어지는가** + 배포 순서 | 계약을 건드릴 때 / PR 리뷰 |
| [05-invariants.md](05-invariants.md) | 절대 불변식 6개와 **코드상 강제 지점**, 규제·신뢰 리스크 | 처음, 그리고 기능 제안 때마다 |
| [06-testing.md](06-testing.md) | 하니스 5종 책임 분담표, verify-all 12항목, **CI 배포 게이트**, 커버리지 공백 G1~G10, 테스트 추가 규칙 | 테스트를 쓸 때 |
...
```bash
# 검증
bash harness/verify-all.sh                       # 전체 12/12
node harness/prepaid.e2e.js                      # 음식점 앱 e2e
node harness/phase2.e2e.mjs                      # 서버 목 하니스 (git push 시 CI 배포 게이트와 같은 명령)
